...
### `recalc_job` はスライス1未デプロイでは通らない

`recalc_job.js` は先頭で BigQuery の `ad_creative_daily` を読むため、
スライス1が未デプロイだと `Not found: Table ...` で落ち、**台帳の再計算まで到達しない**。
台帳の採点自体は BigQuery を必要としない（広告シグナルが空でも計算できる）ので、
広告取込と台帳再計算が同じジョブに同居している点は見直す余地がある。

第1号では `lib/kizuki/ledger-store` の `buildWordRows` / `buildLedgerScoreUpdate` を使って
再計算部分だけを実行した（スコアの正は `lib/kizuki/score.js` のまま・不改変）。
