...
### 4.1.1 全42ページ×PC/スマホの全量翻訳結果（2026-07-16 無料版で実施）

**全42URL（固定ページ20＋FAQ22）をPC・スマホ両UAでクロールし、356文字列を韓国語化して検証。**

| 残存日本語の内訳 | 件数 | 判定 |
|---|---|---|
| `<title>`タグ | 18 | SEO Pack（有料）の領域。既知 |
| WP標準「サンプルページ」 | 9 | 未使用ページ。**削除推奨**（翻訳不要） |
| フォーム送信値（`value`属性と隣接するラベル） | 2 | **意図的スキップが正しい**。訳すとGoogleフォームへの送信値が壊れる |
| HTMLコメント内の無効化済みブロック | 1 | 画面に表示されない。対応不要 |
| **要対応の実コンテンツ** | **0** | ✅ |

- **表示は韓国語・送信値は日本語**が正しく両立している（例: `<option value="大分県">오이타현</option>`）。
  フォーム送信の互換性が保たれることを実証。
- **G3: TCDテーマはPC用とスマホ用でヒーロー文言を別々に出力する**。しかも`<br>`の位置が違うため
  TranslatePressからは別文字列に見える（PC「Pamunは美容ブランドに特化したモニターサービスです。」＝1件 /
  スマホ「Pamunは美容ブランドに特化した」＋「モニターサービスです。」＝2件）。
  → **翻訳の網羅確認は必ずPC・スマホ両方のUAで行うこと**。デスクトップだけで確認すると
  スマホで日本語が残る。（本件は実際に発生し、山口さんの指摘で発覚）

### 4.1.2 画像内テキストの扱い（2026-07-16 調査）

**pamun.jpの画像はほぼ全てが「文字なしのビジュアル素材」であり、画像翻訳の必要はほぼ無い。**

- 実査結果: トップの実コンテンツ画像は11枚、うち文字入りは0枚。
  応募ページも8枚中0枚（コスメのモックアップ写真、背景グラデーション、ロゴのみ）。
  `bizarria-clinic` に至っては実体画像0枚。
- サイトの訴求文言は**画像ではなくHTMLテキストで組まれている**ため、TranslatePressで訳せる。
  TCD系の美容サイトとしては珍しく有利な構造。
- **例外はロゴ**（`Pamun-logo.png`）だが、ブランド名は多言語でも共通表記が通例のため翻訳不要。

**方針: 画像の韓国語版作成は当面スコープ外（YAGNI）。** 将来、韓国語向けに文字入りバナーが
必要になった場合は、④バナークリエイティブ量産（Claude が HTML/SVG 生成 → ヘッドレスでPNG化）の
仕組みをそのまま流用でき、言語別バリエーション出力は①③④の既定路線に含まれる。
ただし以下は運用として実施する:
- **画像の `alt` 属性は翻訳する**（TranslatePressで翻訳可能。韓国語SEOとアクセシビリティに効く）
- 新規に文字入り画像を作る場合は、**文字をHTML側に出す**（画像に焼き込まない）ことを制作ルールにする
