...
> 実際にどこまで拾えるかは Task 3 のカバレッジ実測で確定させる。ここが本プランの検証の核。

## 5. 既知のリスク・要検証事項

| # | リスク | 対応 |
|---|--------|------|
| R1 | TCD087のテーマオプション文言が実際には拾えない箇所がある | Task 3で全量実測。残存箇所は個別に文字列翻訳またはテーマ側で対処 |
| R2 | `super-progressive-web-apps`（PWA）のキャッシュが言語違いのページを混線配信する可能性 | Task 3で `/ko/` を実機確認。混線時はPWAのキャッシュ除外設定または一時停止 |
| R3 | CF7多段フォーム／MW WP Formの確認画面・リダイレクト先が韓国語で破綻する可能性 | Task 3でフォーム送信まで通しで確認 |
