...
- **X（旧Twitter）の自動取込・自動診断（→S2d）**。S2bではXは媒体の選択肢として手動追加／「DBに登録」で保存するのみ。
  Astream X CSV（列：アカウント名/フォロワー数/エンゲージメント/平均いいね・コメント・リポスト・インプレッション/プロフィール 等）の
  一括取込やApifyリプライ転換質はS2dで設計。XはAstream/Apifyとも客層・転換質が取れない構造的限界がある
