...
function collabScore(c) {
  ...
  // 実売(sales>0)は適合スコアが無くても訴求の証拠なので、fit未測定でも+0.2で小スコアを残す。
...
/**
 * 虚栄控除 -20..0。言及が満点上限(MENTION_GOOD)を超える(>MENTION_GOOD)のに
 * 広告で転換が確認できない場合に減点。「パケ可愛い」型（注目は集めるが売れない）を弾く。
 * 言及が上限ちょうど(=MENTION_GOOD)までは正常範囲とみなし控除しない。広告未測定なら控除しない。
 */
function vanityPenalty(signals) {
  ...
  const ctr = num(a.ctr);
  if (mentions <= MENTION_GOOD || ctr === null) return 0;
 ...
 * @param {object} [signals.workshop] - 勉強会シグナル { mentions, brandUnaware }
 * @param {object} [signals.review]   - モニターシグナル { intentRate }
 * @param {object} [signals.ad]       - 広告シグナル { ctr, cvr, roas, demoClarity }（CTR/CVR/ROAS＋デモグラ明確度）
 * @param {object} [signals.collab]   - コラボ実績 { fitScore, sales }
