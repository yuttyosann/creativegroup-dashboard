...
/**
 * ヘッダー行から各項目の列位置を決める。1列は1項目にしか割り当てない。
 *
 * 名前で特定できた列を優先し、特定できなかった項目だけ S のフォールバックを使う
 * （記号だけのヘッダー等、名前から判別できない様式のため）。
 * ただし **フォールバック先が既に名前で確定している列なら使わない**。
 * 例: フォームの生回答シートは先頭にタイムスタンプ等が入り年齢が4列目になるが、
 * 設問の無い improvements/containerWish/favorite が S の 3/4/5 をそのまま使うと
 * age/goodPoints/satisfaction の列を横取りし、年齢や満足度が改善点として
 * LLM に渡ってしまう。該当なしは undefined（＝空文字）に落とす。
 */
function detectSurveyColumns(headerRow) {
  ...
  for (const field of Object.keys(SURVEY_COLUMN_KEYWORDS)) {
    ...
    if (i >= 0) { out[field] = i; taken.add(i); } else { missing.push(field); }
  }
  for (const field of missing) {
    out[field] = taken.has(S[field]) ? undefined : S[field];
  }
  return out;
}
...
function parseSurveyRows(rows) {
  ...
  const answerCols = Object.keys(SURVEY_COLUMN_KEYWORDS)
    .map((f) => cols[f]).filter((i) => i !== undefined);
  const out = [];
  for (const r of (rows || []).slice(1)) {
    // 「設問の列に1つでも回答があるか」で判定する。どこかに値があれば回答者、にはしない。
    // 空白のみのセルも「空」扱い（幽霊回答者が共感率の分母nを水増しするのを防ぐ）。0は有意な値なので残る。
    // 生の回答シートには氏名だけ入って設問が全て空の名簿行が混ざることがあり、
    // それを1人と数えると n が膨らんで購買意向共感率が実際より低く出る。
    if (!r || !answerCols.some((i) => {
      const c = r[i];
